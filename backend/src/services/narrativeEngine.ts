...
import { WORLD_BIBLE, describeBloodGauge } from '../data/worldBible.js'
...
export function buildNarrativePrompt(
  ...
  const archetype = safeString((context.playerProfile as any)?.archetype, '')
  const bloodGauge = Number((context.playerProfile as any)?.bloodGauge ?? 0)

  // System prompt with World Bible as foundation
  const systemPromptRU = `${WORLD_BIBLE}

=== ТЕКУЩАЯ СЕССИЯ ===
${archetype ? `Архетип персонажа: ${archetype}` : ''}
${bloodGauge > 0 ? `Демоническое состояние: ${describeBloodGauge(bloodGauge)}` : ''}
...
4. 📋 ВЫБОРЫ (CHOICES):
   ...
   ✅ Каждый выбор должен быть ясным и соответствовать ситуации
   ❌ ЗАПРЕЩЕНО: менее 3 выборов

5. 👥 ПЕРСОНАЖИ И ДИАЛОГИ:
...
6. 🎭 СТРУКТУРА НАРРАТИВА:
   ✅ 2-4 абзаца: атмосфера → действие/NPC → открытый момент
   ✅ Сенсорные детали из лора мира (запахи, звуки, свет)
...

{
  ...
  "narration": "2-4 абзаца нарратива",
  "choices": ["Действие 1", "Действие 2", "Действие 3"]
}

ПРАВИЛА ПОЛЕЙ:
- "scene.location": ДОЛЖНА совпадать с Context → Location
- "scene.time": короткое время суток на русском
- "narration": только нарратив, без метаязыка
- "choices": РОВНО 3 конкретных действия`
